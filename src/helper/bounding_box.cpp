...
void BoundingBox::GetProbVector(std::vector<float>* bbox_prob) const {
  if(p_ > 0) {
    bbox_prob->push_back(1.0f);
  }else {
    bbox_prob->push_back(0.0f);
  }
}
...
void BoundingBox::Recenter(const BoundingBox& search_location,
  ...
  bbox_gt_recentered->p_  = search_location.p_;
}
...
void BoundingBox::DrawPText(cv::Mat* image) const {
    ...
    ss<<"P:"<< setprecision(2) << p_;
    cv::Point point(x1_,y1_);
    putText(*image, ss.str(), point, fontFace, fontScale, cv::Scalar(255, 255, 255), fontThickness, 16);
}
