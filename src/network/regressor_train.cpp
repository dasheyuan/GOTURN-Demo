...
void RegressorTrain::set_bboxes_gt(const std::vector<BoundingBox>& bboxes_gt) {
  ...
  Blob<float>* input_prob = net_->input_blobs()[3];
  const size_t num_images = bboxes_gt.size();
  const int bbox_dims = 4;
  const int prob_dims = 1;
  vector<int> shape;
  shape.push_back(num_images);
  shape.push_back(bbox_dims);
  input_bbox->Reshape(shape);
  vector<int> prob_shape;
  prob_shape.push_back(num_images);
  prob_shape.push_back(prob_dims);
  input_prob->Reshape(prob_shape);

  // Get a pointer to the bbox memory.
  float* input_bbox_data = input_bbox->mutable_cpu_data();
  float* input_prob_data = input_prob->mutable_cpu_data();
  ...
  int input_prob_data_counter = 0;
  for (size_t i = 0; i < bboxes_gt.size(); ++i) {
    const BoundingBox& bbox_gt = bboxes_gt[i];

    std::vector<float> prob_vect;
    bbox_gt.GetProbVector(&prob_vect);

    input_prob_data[input_prob_data_counter] = prob_vect[0];
    input_prob_data_counter++;
  }
}

void RegressorTrain::set_prob_gt(const std::vector<BoundingBox>& bboxes_gt) {
  assert(net_->phase() == caffe::TRAIN);

  // Reshape the bbox.
  Blob<float>* input_prob = net_->input_blobs()[2];
  const size_t num_images = bboxes_gt.size();
  const int prob_dims = 1;
  vector<int> prob_shape;
  prob_shape.push_back(num_images);
