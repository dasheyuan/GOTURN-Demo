...
void TrackerTrainer::Train(const cv::Mat& image_prev, const cv::Mat& image_curr,
  ...
  //std::vector<cv::Mat> targets;
  //std::vector<BoundingBox> bboxes_gt_scaled;
  //images.push_back(image_curr);
 // targets.push_back(image_prev);
 // bboxes_gt_scaled.push_back(bbox_curr);
  ...
  //while (images.size() > 0) {
    ...
    images_batch_.push_back(image_curr);
    targets_batch_.push_back(image_prev);
    bboxes_gt_scaled_batch_.push_back(bbox_curr);
    ...
    if (images_batch_.size() == kBatchSize) {
      ...
      num_batches_++;
       ...
       /* printf("images batch size : %d\n", images_batch_.size());
        for (size_t i = 0; i < images_batch_.size(); ++i) {
            const cv::Mat &img = images_batch_[i];
            printf("i : %d\n", i);
