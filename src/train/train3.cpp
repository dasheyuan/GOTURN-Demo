...
// Train on a random image.
    void train_image(const LoaderMSCeleb &image_loader,
                     const std::vector<MSCelebAnnotation> &images,
        ...
        int image_num = rand() % (images.size());
        int image_num2 = rand() % (images.size());
        ...
        cv::resize(image,img_r,cv::Size(227, 227), (0, 0), (0, 0), cv::INTER_LINEAR);
        cv::resize(image2,img2_r,cv::Size(227, 227), (0, 0), (0, 0), cv::INTER_LINEAR);


        if(!image_loader.isTheSameDir(image_num,image_num2))
        {
           /* cv::imshow("1", img_r);
            cv::imshow("2", img2_r);
            cv::waitKey(0);*/
...
int main(int argc, char *argv[]) {
    ...
    int arg_index = 1;
    const string &videos_folder_msceleb = argv[arg_index++];
    const string &annotations_folder_msceleb = argv[arg_index++];
    ...
    // Load the image data.
    LoaderMSCeleb image_loader(videos_folder_msceleb, annotations_folder_msceleb);
    const std::vector<MSCelebAnnotation>&train_images = image_loader.get_images();
