...
void LoaderImagenetDet::LoadAnnotationFile(const string& annotation_file,
  ...
  for (TiXmlNode *object = annotations->FirstChild("object"); object; object = object->NextSibling("object")) {
    ...
    // Get the negative boudning box coordinates.
    TiXmlElement *nbbox = object->FirstChildElement("nbndbox");
    if(nbbox)
    {
      const int nxmin = atoi(nbbox->FirstChildElement("xmin")->GetText());
      const int nxmax = atoi(nbbox->FirstChildElement("xmax")->GetText());
      const int nymin = atoi(nbbox->FirstChildElement("ymin")->GetText());
      const int nymax = atoi(nbbox->FirstChildElement("ymax")->GetText());

      // Convert the annotation to bounding box format.

      annotation.nbbox.x1_ = nxmin;
      annotation.nbbox.x2_ = nxmax;
      annotation.nbbox.y1_ = nymin;
      annotation.nbbox.y2_ = nymax;
      annotation.nbbox.p_ = 0;
    }
...
void LoaderImagenetDet::LoadAnnotation(const size_t image_num,
                                       const size_t annotation_num,
                                       cv::Mat *image,
                                       BoundingBox *bbox, BoundingBox *nbbox) const {
  // Load the specified image's annotations.
  const std::vector<Annotation> &annotations = images_[image_num];
  const Annotation &annotation = annotations[annotation_num];
